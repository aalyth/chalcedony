...
impl Span {
    ...
    fn context_substr(&self, pos_: &Position, len: usize) -> (String, usize) {
        ...
        if pos.ln > self.src.len()  { panic!("Error: span: context_substr: position out of bounds."); } 
        if pos.col > self.src[pos.ln].len() { panic!("Error: span: context_substr: position out of bounds."); } 
