...
impl NodeProg {
    pub fn new(mut chunk: VecDeque<Line>, spanner: Rc<dyn Spanning>) -> Result<Self, ChalError> {
        ...
        match front_tok.kind {
            TokenKind::Keyword(Keyword::Let) => {
                ...
                NodeProg::var_def(front_line, spanner)
            }
            TokenKind::Keyword(Keyword::Fn) => NodeProg::func_def(chunk, spanner),
    ...
    #[inline]
    fn var_def(chunk: VecDeque<Token>, spanner: Rc<dyn Spanning>) -> Result<Self, ChalError> {
    ...
    #[inline]
    fn func_def(chunk: VecDeque<Line>, spanner: Rc<dyn Spanning>) -> Result<Self, ChalError> {
        Ok(Self::FuncDef(NodeFuncDef::new(chunk, spanner.clone())?))
    }
}
